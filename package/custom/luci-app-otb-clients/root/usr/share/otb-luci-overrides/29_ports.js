...
function buildVLANMappings(mapping, vlanids)
{
	...
	/* find bridges, bonds and VLAN devices */
	for (let i = 0, s; (s = vlan_devices[i]) != null; i++) {
		...
		else if (s.type == 'bonding' || s.type == 'bond') {
			if (!isString(s.name))
				continue;

			const ports = L.toArray(s.ports || s.slaves);

			if (ports.length)
				mapping[s.name] = ports;
		}
	...
	/*
	 * netifd's legacy proto-bonding represents a bond as an interface rather
	 * than a config device.  The runtime netdev is named "bond-<section>".
	 * Registering this relation is required for the port-status widget to
	 * attribute bridge VLAN networks to the physical bond members.
	 */
	for (let i = 0, s; (s = interfaces[i]) != null; i++) {
		if (s.proto == 'bonding') {
			const section = s['.name'];
			const bondname = isString(s.device) ? s.device :
				(isString(s.ifname) ? s.ifname :
				 (isString(section) ? 'bond-' + section : null));
			const ports = L.toArray(s.slaves || s.ports);

			if (bondname && ports.length)
				mapping[bondname] = ports;
		}
	...
	/* Attribute every bridge VLAN to its final physical ports, through bonds. */
	if (vlanids) {
		for (let brname in bridges) {
			for (let vid in bridges[brname].vlans) {
				for (let member of bridges[brname].vlans[vid]) {
					for (let port of resolveVLANPorts(member, mapping)) {
						vlanids[port] = vlanids[port] || [];

						if (vlanids[port].indexOf(+vid) === -1)
							vlanids[port].push(+vid);
					}
				}
			}
		}

		for (let port in vlanids)
			vlanids[port].sort((a, b) => a - b);
	}
}
...
function buildInterfaceMapping(zones, networks) {
	...
	buildVLANMappings(vlanmap, vlanids);

	for (let p in vlanids)
		portmap[p] = { networks: [], zones: [], vlans: vlanids[p] };
	...
	for (let net of networks) {
		...
		for (let p of ports) {
			portmap[p] = portmap[p] || { networks: [], zones: [], vlans: vlanids[p] || [] };
...
function renderNetworksTooltip(pmap) {
	...
	const vlans = (pmap && Array.isArray(pmap.vlans)) ? pmap.vlans : [];
	...
	if (vlans.length)
		res.push(E('br'), E('strong', {}, [ _('VLAN:') + ' ' ]), vlans.join(', '));

	return E([], res);
}

function renderVisibleVLANs(pmap) {
	const vlans = (pmap && Array.isArray(pmap.vlans)) ? pmap.vlans : [];
	...
	return E('div', {
		'class': 'otb-port-vlans',
		'title': _('VLAN carried by this physical port'),
		'style': 'margin-top:.3em;font-size:72%;font-weight:600;line-height:1.2;overflow-wrap:anywhere'
	}, [ _('VLAN') + ' ' + vlans.join(' · ') ]);
}
...
return baseclass.extend({
	...
	load() {
		return Promise.all([
			...
			/*
			 * getBuiltinEthernetPorts() omet notamment eth1 sur le R4 Pro,
			 * alors qu'il s'agit bien d'un membre physique de bond-trunk. Ajouter
			 * explicitement tous les membres des interfaces proto-bonding permet
			 * de montrer les deux liens LACP et leurs VLAN hérités.
			 */
			for (let section of uci.sections('network', 'interface')) {
				if (section.proto != 'bonding')
					continue;

				const bondname = isString(section.device) ? section.device :
					(isString(section.ifname) ? section.ifname :
					 (isString(section['.name']) ? 'bond-' + section['.name'] : null));

				for (let member of L.toArray(section.slaves || section.ports)) {
					if (!allPorts.has(member)) {
						builtinPorts.push({
							role: 'lan',
							device: member,
							bond: bondname
	...
	render(data) {
		...
		return E('div', { 'style': 'display:grid;grid-template-columns:repeat(auto-fit, minmax(100px, 1fr));margin-bottom:1em;align-items:center;justify-items:center;text-align:center' }, known_ports.map(function(port) {
			...
			const visibleVLANs = renderVisibleVLANs(pmap);
			...
			if (visibleVLANs)
				linkContent.push(visibleVLANs);

			return E('div', { 'class': 'ifacebox', 'style': 'margin:.25em;width:112px' }, [
				E('div', { 'class': 'ifacebox-head', 'style': 'font-weight:bold' }, [ port.netdev.getName() ]),
				E('div', { 'class': 'ifacebox-body' }, linkContent),
